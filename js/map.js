...
window.NEXUS_MAP_ENGINE = (() => {
  ...
  function drawRegions(){
    if(camera.zoom<4||!(state.selectedCountryId==="ESP"||state.controlledCountryId==="ESP"))return;
    if(regionGeojson.features?.length){
      for(const feature of regionGeojson.features){const region=state.regions.find(r=>r.id===feature.properties?.regionId);if(!region)continue;const selected=region.id===state.selectedRegionId;for(const path of geometryPaths(feature.geometry)){ctx.save();ctx.fillStyle=selected?"rgba(255,220,82,.34)":"rgba(45,178,220,.18)";ctx.fill(path);ctx.strokeStyle=selected?"#ffe46a":"#63d6f0";ctx.lineWidth=selected?2.3:1.15;ctx.stroke(path);ctx.restore();hitRegions.push({path,region})}if(camera.zoom>4.6){const p=toScreen(region.lat,region.lng);drawLabel(p.x,p.y,region.name,selected?"#ffe46a":"#e1f8ff",selected?10:8)}}
      return;
    }
    for(const region of state.regions){const p=toScreen(region.lat,region.lng),selected=region.id===state.selectedRegionId;const radius=Math.max(9,Math.min(28,7+region.gdp/18))*Math.pow(1.12,camera.zoom-4);ctx.save();ctx.beginPath();ctx.arc(p.x,p.y,radius,0,Math.PI*2);ctx.fillStyle=selected?"rgba(255,220,82,.32)":"rgba(57,184,221,.16)";ctx.fill();ctx.strokeStyle=selected?"#ffe46a":"#63cae7";ctx.lineWidth=selected?2:1;ctx.stroke();ctx.restore();if(camera.zoom>4.8)drawLabel(p.x,p.y-radius-5,region.name,selected?"#ffe46a":"#d8f4ff",8)}
  }
  ...
  function hover(e){const p=eventPoint(e);const marker=[...hitMarkers].reverse().find(m=>Math.hypot(p.x-m.x,p.y-m.y)<=m.r);if(marker){showTipAt(p.x,p.y,marker.html);canvas.style.cursor="pointer";return}const regionHit=[...hitRegions].reverse().find(h=>ctx.isPointInPath(h.path,p.x,p.y));if(regionHit){const r=regionHit.region;showTipAt(p.x,p.y,`<strong>🗺️ ${r.name}</strong><span>${r.capital} · ${r.population.toLocaleString("es-ES",{maximumFractionDigits:2})} M hab.</span><span>PIB ${r.gdp.toLocaleString("es-ES")} mil M€ · Industria ${r.industry.toFixed(0)}</span><span>Clic para seleccionar la comunidad</span>`);canvas.style.cursor="pointer";return}const hit=[...hitCountries].reverse().find(h=>ctx.isPointInPath(h.path,p.x,p.y));if(hit){const c=hit.country;showTipAt(p.x,p.y,`<strong>${c.flag} ${c.name}</strong><span>PIB ${c.economy.gdp.toLocaleString("es-ES",{maximumFractionDigits:0})} mil M€ · ${c.economy.population.toLocaleString("es-ES",{maximumFractionDigits:1})} M hab.</span><span>Industria ${c.systems.industry.toFixed(0)} · Tecnología ${c.systems.technology.toFixed(0)} · Militar ${c.systems.military.toFixed(0)}</span><span>${c.id===state.controlledCountryId?"CONTROLADO":"Clic para inspeccionar"}</span>`);canvas.style.cursor="pointer";return}hideTip();canvas.style.cursor=dragging?"grabbing":"grab"}
  function clickMap(e){if(dragging)return;const p=eventPoint(e);const marker=[...hitMarkers].reverse().find(m=>Math.hypot(p.x-m.x,p.y-m.y)<=m.r);if(marker){showTipAt(p.x,p.y,marker.html);return}const regionHit=[...hitRegions].reverse().find(h=>ctx.isPointInPath(h.path,p.x,p.y));if(regionHit){callbacks.selectRegion?.(regionHit.region.id);return}const hit=[...hitCountries].reverse().find(h=>ctx.isPointInPath(h.path,p.x,p.y));if(hit)callbacks.selectCountry?.(hit.country.id)}
