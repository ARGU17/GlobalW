...
(() => {
  const SAVE_KEY="nexus_alpha_v1_6_save";
  const LEGACY_KEYS=["nexus_alpha_v1_5_save","nexus_alpha_v1_4_save","nexus_alpha_v1_3_save","nexus_alpha_v1_2_save","nexus_alpha_v1_1_save","nexus_alpha_v1_0_save"];
  const DAY_MS=10000;
  ...
  function createActions(){return{
    ...
    takeControl,changeRegime,appointParty,callElection,negotiateCoalition,removeCoalitionParty,setCompanyPolicy,enactNationalDecision,demandSurrender,annexOccupiedRegions,annexCountry,signPeace,
  ...
  function exportSave(){const blob=new Blob([JSON.stringify(state,null,2)],{type:"application/json"}),url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=`nexus-v1.6-${state.date}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);NEXUS_UI.toast("Guardado exportado.","success")}
