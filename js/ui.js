...
window.NEXUS_UI = (() => {
  ...
  function renderPolitics() {
    ...
    return `${heading("Sistema político","Régimen, partidos, alianzas parlamentarias y transición institucional",`<button data-action="callElection">Convocar elecciones</button>`)}
      <div class="kpi-grid four">${kpi("Régimen",reg.name,`Pluralismo ${reg.pluralism}`)}${kpi("Gobierno",ruling?.name||"—",ruling?.ideology||"")}${kpi("Coalición",pct(coalition.support),coalition.support>=50?"Mayoría política":"Gobierno minoritario",coalition.support>=50?"positive":"warning")}${kpi("Capital político",fmt1(c.politics.politicalCapital),"negociaciones")}</div>
      <section class="card parliament-card"><div class="card-title"><h3>Distribución del poder político</h3><span>180° = 100% del poder nacional · 90° = 50%</span></div>${parliamentChart(c)}<div class="parliament-summary"><b>Coalición: ${coalition.members.map(x=>esc(x.name)).join(" + ")||"sin acuerdo"}</b><span>Apoyo ${pct(coalition.support)} · eje ideológico ${coalition.axis>0?"+":""}${fmt1(coalition.axis)}</span></div></section>
  ...
  function renderMilitary() {
    ...
    return `${heading("Mando de Fuerzas Armadas","Producción por cantidades reales, movimiento regional y conquista territorial")}
      <div class="kpi-grid four">${kpi("Efectivos y material",fmt0(totalUnits(c)),"cantidad total")}${kpi("Preparación",pct(c.militaryReadiness),"nacional")}${kpi("Doctrina",c.militaryDoctrine,"operativa")}${kpi("Operaciones regionales",fmt0(regionalBattles.length),"activas")}</div>
      ...
      <section class="card"><div class="card-title"><h3>Unidades desplegadas</h3><span>Selecciona región propia para mover o región enemiga para atacar</span></div><div class="deployed-grid">${c.units.filter(u=>u.quantity>0).map(deployedUnit).join("")}</div></section>
