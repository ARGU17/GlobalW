...
window.NEXUS_UI = (() => {
  ...
  function renderIndustry() {
    const c=controlled(),regions=E().getCountryRegions?.(state,c.id)||[],regionNames=new Map(regions.map(r=>[r.id,r.name])),facilities=c.id==="ESP"?state.regions.flatMap(r=>r.buildings.map(b=>({...b,place:r.name}))):c.facilities.map(b=>({...b,place:regionNames.get(b.regionId)||c.name}));
  ...
  function renderPolitics() {
    ...
    return `${heading("Sistema político","Régimen, partidos, alianzas parlamentarias y transición institucional",`<button data-action="callElection">Convocar elecciones</button>`)}
      <div class="kpi-grid four">${kpi("Régimen",reg.name,`Pluralismo ${reg.pluralism}`)}${kpi("Gobierno",ruling?.name||"—",ruling?.ideology||"")}${kpi("Coalición",`${coalitionSeats}/350`,majority?"Mayoría absoluta":"Gobierno minoritario",majority?"positive":"warning")}${kpi("Capital político",fmt1(c.politics.politicalCapital),"negociaciones")}</div>
  ...
  function renderMilitary() {
    const c=controlled(),activeWars=state.wars.filter(w=>!w.ended),batch=state.unitBatch||1,regionalBattles=(state.regionBattles||[]).filter(b=>!b.ended&&(b.attackerId===c.id||b.defenderId===c.id));
    ...
    return `${heading("Mando de Fuerzas Armadas","Producción real, despliegue regional, operaciones por flancos y tratados de guerra")}
      <div class="kpi-grid four">${kpi("Efectivos y material",fmt0(totalUnits(c)),"inventario total")}${kpi("Grupos desplegados",fmt0(c.units.filter(u=>u.quantity>0).length),"regiones y flancos")}${kpi("Preparación",pct(c.militaryReadiness),"nacional")}${kpi("Guerras activas",fmt0(activeWars.length),"conflictos")}</div>
      ...
      <section class="card"><div class="card-title"><h3>Unidades desplegadas por regiones</h3><span>Divide grupos para abrir varios flancos; mueve o ataca desde territorios controlados</span></div><div class="deployed-grid">${c.units.filter(u=>u.quantity>0).map(deployedUnit).join("")||`<p class="muted">No hay unidades desplegadas.</p>`}</div></section>
      ...
      <section class="card"><div class="card-title"><h3>Centro de guerra</h3><span>${activeWars.length} conflictos activos</span></div>${activeWars.length?activeWars.map(w=>warCard(w,false)).join(""):`<div class="empty-state"><span>🕊️</span><p>Selecciona un país hostil y declara la guerra desde Diplomacia.</p></div>`}</section>
  ...
  function deployedUnit(u){const c=controlled(),d=unitDef(u.typeId),ownRegions=E().getControlledRegions?.(state,c.id)||(E().getCountryRegions?.(state,c.id)||[]).filter(r=>(r.controllerId||c.id)===c.id),region=E().getRegion?.(state,c.id,u.regionId),enemyOptions=[];for(const w of state.wars.filter(w=>!w.ended&&(w.attacker===c.id||w.defender===c.id))){const enemyId=w.attacker===c.id?w.defender:w.attacker;for(const r of E().getCountryRegions?.(state,enemyId)||[])if((r.controllerId||enemyId)!==c.id)enemyOptions.push({countryId:enemyId,region:r})}const movement=u.movement,progress=movement?clamp((movement.progress||0)*100,0,100):0;return `<article class="deployed-unit"><img src="${d?.icon||""}" alt=""><div><h4>${esc(d?.name||u.name)}</h4><p>${fmt0(u.quantity)} ${esc(d?.unitName||"unidades")} · ${esc(region?.name||c.name)}</p><span>${pct(u.readiness)} · EXP ${fmt0(u.experience)} · ${esc(u.status||"desplegada")}</span>${movement?`<div class="progress"><i style="width:${progress}%"></i></div><small>${fmt0(movement.daysRemaining)} días restantes</small>`:`<div class="unit-orders"><select id="deploy-${u.id}">${ownRegions.map(x=>{const owner=E().getCountry(state,x.countryId||c.id);return `<option value="${x.countryId||c.id}|${x.id}" ${x.id===u.regionId?"selected":""}>Mover: ${owner?.flag||"🏳️"} ${esc(x.name)}</option>`}).join("")}</select><button data-action="moveUnit" data-unit-id="${u.id}">Mover</button>${u.quantity>1?`<input id="splitQty-${u.id}" type="number" min="1" max="${Math.max(1,u.quantity-1)}" value="${Math.max(1,Math.floor(u.quantity/2))}" aria-label="Cantidad a separar"><select id="splitRegion-${u.id}">${ownRegions.map(x=>`<option value="${x.id}" ${x.id===u.regionId?"selected":""}>Destacamento: ${esc(x.name)}</option>`).join("")}</select><button data-action="splitUnit" data-unit-id="${u.id}">Dividir grupo</button>`:""}${enemyOptions.length?`<select id="attack-${u.id}">${enemyOptions.map(x=>`<option value="${x.countryId}|${x.region.id}">Atacar: ${esc(x.region.name)} · ${esc(x.region.resourceProfile?.primary||"recursos")}</option>`).join("")}</select><button class="danger-btn" data-action="attackRegion" data-unit-id="${u.id}">Atacar región</button>`:""}</div>`}</div></article>`}
  ...
  function warCard(w,showSettlement=false){const a=E().getCountry(state,w.attacker),d=E().getCountry(state,w.defender),battle=w.lastBattle,opts=E().warSettlementOptions?.(state,w.id),participant=(w.attacker===controlled().id||w.defender===controlled().id),adv=opts?.advantage||0,occupied=opts?.occupied||[];return `<article class="war-card ${w.ended?"ended":"active"}"><header><div><h3>${a.flag} ${esc(a.name)} <span>vs</span> ${d.flag} ${esc(d.name)}</h3><p>Día ${w.days||0} · Control ${fmt1(w.territoryControl||0)} · War score ${fmt1(w.warScore||0)} · Ventaja propia ${adv>=0?"+":""}${fmt1(adv)}</p></div><b>${w.result||(w.ended?"CONFLICTO FINALIZADO":"GUERRA ACTIVA")}</b></header>${warCompact(w)}<div class="battle-grid"><div><h4>Fuerza atacante</h4>${forceComposition(a)}</div><div><h4>Fuerza defensora</h4>${forceComposition(d)}</div></div>${battle?`<div class="battle-report"><h4>⚔️ ${esc(battle.title)}</h4><p>${esc(battle.summary)}</p><div>${(battle.attackerUnits||[]).map(x=>`<span>${esc(x)}</span>`).join("")}</div><div>${(battle.defenderUnits||[]).map(x=>`<span>${esc(x)}</span>`).join("")}</div><small>Bajas del último parte: ${fmt0(battle.attackerLosses)} / ${fmt0(battle.defenderLosses)}</small></div>`:`<p class="muted">La primera batalla se generará al avanzar el día.</p>`}<footer><span>Bajas acumuladas: ${fmt0(w.attackerLosses)} / ${fmt0(w.defenderLosses)} · regiones ocupadas ${occupied.length}</span>${participant?`<div class="war-actions">${!w.ended?`<button data-action="signPeace" data-war-id="${w.id}">Paz negociada</button>${opts?.canDemand?`<button class="primary-btn" data-action="demandSurrender" data-war-id="${w.id}">Exigir capitulación</button>`:""}`:""}${opts?.canAnnexOccupied?`<button data-action="annexOccupied" data-war-id="${w.id}">Anexar regiones ocupadas</button>`:""}${opts?.canAnnexCountry?`<button class="danger-btn" data-action="annexCountry" data-war-id="${w.id}">Anexar país</button>`:""}${w.ended&&!w.settlement?.resolved?`<button data-action="signPeace" data-war-id="${w.id}">Cerrar con paz</button>`:""}</div>`:""}</footer>${showSettlement&&w.settlement?`<div class="settlement-summary"><b>Tratado: ${esc(w.settlement.type||w.settlement.status||"pendiente")}</b><span>${w.settlement.date||"Decisión pendiente"}</span></div>`:""}</article>`}
